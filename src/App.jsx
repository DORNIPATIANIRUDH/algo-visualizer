...
function stBubble(inp) {
  ...
  for(let i=0;i<n-1;i++){
    ...
    done.add(n-1-i);out.push({a:[...a],hi:[],sw:[],done:[...done],msg:`Pass ${i+1} done — index ${n-1-i} locked in`,ln:2});
  }
  done.add(0);out.push({a:[...a],hi:[],sw:[],done:[...done],msg:"✓ Array fully sorted!",ln:8});return out;
}
function stInsertion(inp) {
  ...
  for(let i=1;i<n;i++){
    ...
    a[j+1]=key;out.push({a:[...a],hi:[],sw:[j+1],done:Array.from({length:i+1},(_,k)=>k),msg:`Inserted key=${key} at index ${j+1}`,ln:7});
  }
  out.push({a:[...a],hi:[],sw:[],done:Array.from({length:n},(_,k)=>k),msg:"✓ Array fully sorted!",ln:9});return out;
}
function stMerge(inp) {
  ...
  function ms(arr,l,r){
    if(l<r){const m=Math.floor((l+r)/2);out.push({a:[...arr],hi:[l,r],sw:[],done:[],msg:`Split [${l}..${r}] at mid=${m}`,ln:3});ms(arr,l,m);ms(arr,m+1,r);merge(arr,l,m,r);}
  }
  ms(a,0,a.length-1);
  out.push({a:[...a],hi:[],sw:[],done:Array.from({length:a.length},(_,k)=>k),msg:"✓ Array fully sorted!",ln:17});return out;
}
...
function stFibRec(n){
  ...
  function fib(k){
    calls++;out.push({active:k,memo:{},table:null,calls,result:null,msg:`fib(${k}) called — total calls: ${calls}`,ln:1});
  ...
  const res=fib(n);
  out.push({active:-1,memo:{},table:null,calls,result:res,msg:`✓ fib(${n}) = ${res} — needed ${calls} calls!`,ln:4});return out;
}
function stFibMemo(n){
  ...
  const res=fib(n);
  out.push({active:-1,memo:{...memo},table:null,calls:0,result:res,msg:`✓ fib(${n}) = ${res} — ${Object.keys(memo).length} unique computations`,ln:8});return out;
}
function stFibDP(n){
  ...
  out.push({active:-1,memo:{},table:[...dp],calls:0,result:dp[n],msg:`✓ fib(${n}) = dp[${n}] = ${dp[n]}`,ln:6});return out;
}
...
const ALGOS = [
  { id:"sorting", label:"Sorting", vizType:"bars", approaches:[
  ...
  { id:"searching", label:"Searching", vizType:"array", approaches:[
  ...
  { id:"twosum", label:"Two Sum", vizType:"array", approaches:[
  ...
  { id:"fibonacci", label:"Fibonacci", vizType:"fib", approaches:[
...
function BarViz({ step }) {
  ...
  return (
    <div style={{ display:"flex", alignItems:"flex-end", gap:2, height:140, padding:"0 6px", width:"100%" }}>
      {a.map((v, i) => {
        let bg = C.border;
        if (done.includes(i)) bg = C.opt;
        else if (sw.includes(i)) bg = C.brute;
        else if (hi.includes(i)) bg = C.better;
        const h = Math.max(8, Math.round((v / max) * 128));
        return (
    ...
    return (
      <div style={{ padding:"4px 8px", width:"100%" }}>
        ...
        <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
          {table.slice(0, Math.min(table.length,12)).map((v, i) => {
        ...
        <div>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:4 }}>
            {memoKeys.slice(0,14).map(k => (
              <div key={k} style={{ padding:"2px 7px", background:+k===active?"#0e1a2a":C.s2, border:`1px solid ${+k===active?C.accent:C.border}`, borderRadius:4, fontFamily:"JetBrains Mono", fontSize:10, color:C.text }}>{k}:{memo[k]}</div>
            ))}
          </div>
        </div>
      )}
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", background:C.bg }}>
      <div style={{ display:"flex", borderBottom:`1px solid ${C.border}` }}>
        {LANGS.map(l => (
          <button key={l} onClick={() => onLang(l)} style={{ flex:1, padding:"5px 4px", background:language===l?C.s1:"transparent", color:language===l?C.text:C.muted, border:"none", borderBottom:language===l?`2px solid ${C.accent}`:"2px solid transparent", cursor:"pointer", fontSize:10, fontFamily:"JetBrains Mono", transition:"all 0.12s" }}>
            {LANG_LABELS[l]}
          </button>
        ))}
      </div>
...
function Ctrl({ onClick, title }) {
  return (
    <button onClick={onClick} style={{ padding:"5px 10px", background:C.s2, border:`1px solid ${C.border}`, color:C.muted, borderRadius:5, cursor:"pointer", fontSize:13, fontFamily:"JetBrains Mono", touchAction:"manipulation" }}>
      {title}
    </button>
  );
}
...
export default function AlgoVisualizer() {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100dvh", background:C.bg, color:C.text, fontFamily:"Syne, sans-serif", overflow:"hidden", fontSize:13, position:"relative" }}>

      {/* ── HEADER ── */}
      <div style={{ display:"flex", alignItems:"center", padding:"7px 14px", borderBottom:`1px solid ${C.border}`, background:C.s1, gap:10, flexShrink:0, zIndex:50 }}>
        {!isMobile && (
        ...
        {isMobile && <span style={{ fontFamily:"Syne", fontWeight:800, fontSize:16, letterSpacing:-0.3 }}>AlgoViz</span>}
        <div style={{ flex:1 }} />
        ...
        {cat.id!=="fibonacci" && (
          <button onClick={randomize} style={{ padding:"3px 9px", background:C.s2, border:`1px solid ${C.border}`, color:C.muted, borderRadius:4, cursor:"pointer", fontSize:isMobile?14:11, fontFamily:"Syne", touchAction:"manipulation" }}>
            {isMobile?"🔀":"Randomize"}
          </button>
        )}
        {isMobile && (
          <button onClick={()=>setMenuOpen(o=>!o)} style={{ padding:"4px 10px", background:menuOpen?C.s2:"transparent", border:`1px solid ${menuOpen?C.border:"transparent"}`, color:C.text, borderRadius:4, cursor:"pointer", fontSize:17, touchAction:"manipulation" }}>☰</button>
        )}
      </div>

      {/* ── MOBILE DROPDOWN MENU ── */}
      {isMobile && menuOpen && (
        <div onClick={()=>setMenuOpen(false)} style={{ position:"fixed", inset:0, zIndex:150, background:"rgba(0,0,0,0.55)" }}>
          <div onClick={e=>e.stopPropagation()} style={{ position:"absolute", top:46, left:0, right:0, background:C.s1, borderBottom:`1px solid ${C.border}`, zIndex:200, padding:"8px 0" }}>
            <div style={{ padding:"4px 12px 2px", fontSize:9, fontWeight:700, letterSpacing:"0.1em", color:C.muted, textTransform:"uppercase" }}>Category</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:6, padding:"6px 12px 10px" }}>
              {ALGOS.map((a, i) => (
                <button key={a.id} onClick={()=>{setCatIdx(i);setAppIdx(0);setMenuOpen(false);}} style={{ padding:"6px 12px", background:catIdx===i?`${C.accent}22`:C.s2, border:`1px solid ${catIdx===i?C.accent:C.border}`, color:catIdx===i?C.accent:C.muted, borderRadius:6, cursor:"pointer", fontSize:12, fontFamily:"Syne", touchAction:"manipulation" }}>
                  {a.label}
                </button>
              ))}
            </div>
            <div style={{ padding:"4px 12px 2px", fontSize:9, fontWeight:700, letterSpacing:"0.1em", color:C.muted, textTransform:"uppercase" }}>Approach</div>
            <div style={{ display:"flex", gap:6, padding:"6px 12px 10px" }}>
              {cat.approaches.map((a, i) => {
                const tc = TIER[a.tier].color;
                return (
                  <button key={a.id} onClick={()=>{setAppIdx(i);setMenuOpen(false);}} style={{ flex:1, padding:"7px 4px", background:appIdx===i?`${tc}22`:C.s2, border:`1px solid ${appIdx===i?tc:C.border}`, color:appIdx===i?tc:C.muted, borderRadius:6, cursor:"pointer", fontSize:10, fontFamily:"Syne", textAlign:"center", touchAction:"manipulation" }}>
      ...
      <div style={{ display:"flex", borderBottom:`1px solid ${C.border}`, background:C.s1, flexShrink:0 }}>
        {cat.approaches.map((a, i) => {
          const tc = TIER[a.tier].color;
          return (
            <button key={a.id} onClick={()=>setAppIdx(i)} style={{ flex:1, padding:"7px 6px", background:appIdx===i?C.bg:"transparent", color:appIdx===i?tc:C.muted, border:"none", borderBottom:appIdx===i?`2.5px solid ${tc}`:"2.5px solid transparent", cursor:"pointer", fontSize:isMobile?10:11, fontFamily:"Syne", fontWeight:appIdx===i?700:400, transition:"all 0.12s", touchAction:"manipulation" }}>
              {TIER[a.tier].label}
              <span style={{ fontSize:9, marginLeft:4, opacity:0.75 }}>{a.time}</span>
            </button>
          );
        })}
      </div>

      {/* ── MOBILE VIZ/CODE TABS ── */}
      {isMobile && (
        <div style={{ display:"flex", borderBottom:`1px solid ${C.border}`, background:C.s1, flexShrink:0 }}>
          {["viz","code"].map(t => (
      ...
      <div style={{ flex:1, display:"flex", overflow:"hidden", minHeight:0 }}>
        ...
        {!isMobile && (
          <div style={{ width:180, background:C.s1, borderRight:`1px solid ${C.border}`, display:"flex", flexDirection:"column", overflowY:"auto", flexShrink:0 }}>
            <div style={{ padding:"14px 12px 6px", fontSize:15, fontWeight:800, color:C.text, letterSpacing:-0.3 }}>AlgoViz</div>
            ...
            <div style={{ padding:"8px 12px", borderTop:`1px solid ${C.border}` }}>
              <div style={{ fontSize:9, color:C.muted, marginBottom:4, textTransform:"uppercase", letterSpacing:"0.08em" }}>Legend</div>
              {cat.vizType==="bars" ? <>
        ...
        <div style={{ flex:1, display:"flex", overflow:"hidden", minHeight:0 }}>
          ...
          {/* VIZ PANEL */}
          {(!isMobile || tab==="viz") && (
            <div style={{ flex:isMobile?1:1.1, display:"flex", flexDirection:"column", borderRight:isMobile?"none":`1px solid ${C.border}`, minWidth:0 }}>
              ...
              <div style={{ padding:"6px 10px", display:"flex", alignItems:"center", gap:5, background:C.s1, borderTop:`1px solid ${C.border}`, flexShrink:0 }}>
                <Ctrl onClick={()=>{setStepIdx(0);setPlaying(false);}} title="⏮" />
                <Ctrl onClick={()=>setStepIdx(s=>Math.max(0,s-1))} title="◀" />
                <button onClick={()=>setPlaying(p=>!p)} style={{ padding:"5px 16px", background:playing?C.s2:`${tierColor}22`, border:`1px solid ${playing?C.border:tierColor}`, color:playing?C.muted:tierColor, borderRadius:5, cursor:"pointer", fontSize:14, fontWeight:600, fontFamily:"JetBrains Mono", touchAction:"manipulation" }}>
                  {playing?"⏸":"▶"}
                </button>
                ...
                <input type="range" min={80} max={1200} step={80} value={speed} onChange={e=>setSpeed(+e.target.value)} style={{ width:65, accentColor:tierColor }} />
              </div>
