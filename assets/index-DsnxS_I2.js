...
// O(log n) query!`,`class SegTree:
  ...
  for(let i=1;i<n-1;i++){
    ...
    for i in range(1,len(h)-1):
        ...
        tot+=min(l,r)-h[i]
    return tot`,`int trap(vector<int>&h){int tot=0,n=h.size();for(int i=1;i<n-1;i++){int l=*max_element(h.begin(),h.begin()+i+1),r=*max_element(h.begin()+i,h.end());tot+=min(l,r)-h[i];}return tot;}`,`int trap(int[]h){int tot=0,n=h.length;for(int i=1;i<n-1;i++){int l=0,r=0;for(int k=0;k<=i;k++)l=Math.max(l,h[k]);for(int k=i;k<n;k++)r=Math.max(r,h[k]);tot+=Math.min(l,r)-h[i];}return tot;}`),trapTwoPtr:A(`function trap(h){
  ...
  while(l<r){
    ...
    return tot`,`int trap(vector<int>&h){int l=0,r=h.size()-1,mL=0,mR=0,tot=0;while(l<r){if(h[l]<=h[r]){mL=max(mL,h[l]);tot+=mL-h[l];l++;}else{mR=max(mR,h[r]);tot+=mR-h[r];r--;}}return tot;}`,`int trap(int[]h){int l=0,r=h.length-1,mL=0,mR=0,tot=0;while(l<r){if(h[l]<=h[r]){mL=Math.max(mL,h[l]);tot+=mL-h[l];l++;}else{mR=Math.max(mR,h[r]);tot+=mR-h[r];r--;}}return tot;}`),histStack:A(`function largestRect(h){
  ...
  for(let i=0;i<=h.length;i++){
    ...
    for i in range(len(h)+1):
        ...
        stack.append(i)
    return mx`,`int largestRect(vector<int>&h){stack<int>st;int mx=0,n=h.size();for(int i=0;i<=n;i++){while(!st.empty()&&(i==n||h[st.top()]>=h[i])){int ht=h[st.top()];st.pop();int w=st.empty()?i:i-st.top()-1;mx=max(mx,ht*w);}st.push(i);}return mx;}`,`int largestRect(int[]h){Deque<Integer>st=new ArrayDeque<>();int mx=0,n=h.length;for(int i=0;i<=n;i++){while(!st.isEmpty()&&(i==n||h[st.peek()]>=h[i])){int ht=h[st.pop()];int w=st.isEmpty()?i:i-st.peek()-1;mx=Math.max(mx,ht*w);}st.push(i);}return mx;}`),ngeStack:A(`function dailyTemps(T){
  ...
  for(let i=0;i<T.length;i++){
    ...
    for i,t in enumerate(T):
        ...
        stack.append(i)
    return res`,`vector<int>dailyTemperatures(vector<int>&T){int n=T.size();vector<int>res(n);stack<int>st;for(int i=0;i<n;i++){while(!st.empty()&&T[st.top()]<T[i]){res[st.top()]=i-st.top();st.pop();}st.push(i);}return res;}`,`int[]dailyTemperatures(int[]T){int n=T.length;int[]res=new int[n];Deque<Integer>st=new ArrayDeque<>();for(int i=0;i<n;i++){while(!st.isEmpty()&&T[st.peek()]<T[i])res[st.peek()]=i-st.pop();st.push(i);}return res;}`)},cr={pathBrute:A(`function uniquePaths(m,n){
  function f(r,c){
    if(r>=m||c>=n)return 0;
    ...
    def f(r,c):
        ...
        return f(r+1,c)+f(r,c+1)
    return f(0,0)`,`int uniquePaths(int m,int n){function<int(int,int)>f=[&](int r,int c)->int{if(r>=m||c>=n)return 0;if(r==m-1&&c==n-1)return 1;return f(r+1,c)+f(r,c+1);};return f(0,0);}`,`int uniquePaths(int m,int n){return solve(0,0,m,n);}int solve(int r,int c,int m,int n){if(r>=m||c>=n)return 0;if(r==m-1&&c==n-1)return 1;return solve(r+1,c,m,n)+solve(r,c+1,m,n);}`),pathDP:A(`function uniquePaths(m,n){
  ...
  return dp[m-1][n-1];}`,`def uniquePaths(m,n):
    ...
    for r in range(1,m):
        for c in range(1,n):
            dp[r][c]=dp[r-1][c]+dp[r][c-1]
    return dp[m-1][n-1]`,`int uniquePaths(int m,int n){vector<vector<int>>dp(m,vector<int>(n,1));for(int r=1;r<m;r++)for(int c=1;c<n;c++)dp[r][c]=dp[r-1][c]+dp[r][c-1];return dp[m-1][n-1];}`,`int uniquePaths(int m,int n){int[][]dp=new int[m][n];for(int[]r:dp)Arrays.fill(r,1);for(int r=1;r<m;r++)for(int c=1;c<n;c++)dp[r][c]=dp[r-1][c]+dp[r][c-1];return dp[m-1][n-1];}`),minPath:A(`function minPathSum(g){
